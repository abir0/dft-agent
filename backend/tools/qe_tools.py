...
def _write_qe_input_with_ase(
    ...
    aseio.write(str(inpath), atoms, format="espresso-in", **kw)  # QE input writer in ASE
    return str(inpath)
...
def _parse_scf(out_text: str) -> Dict[str, Any]:
    ...
    """
    data: Dict[str, Any] = {"converged": False, "etot": None, "nbands": None, "warnings": []}
    ...
    for pat in [r"convergence NOT achieved", r"too many bands are not converged",
                r"Exit code:\s*\d+", r"error", r"WARNING"]:
...
def _tune_params_on_failure(params: QEParams, out_text: str) -> QEParams:
    ...
    if "convergence not achieved" in txt or "error" in txt:
        ...
        new.electrons["electron_maxstep"] = max(int(new.electrons.get("electron_maxstep", 100)) * 2, 200)
...
def prepare_qe_inputs(
    ...
    job = JobSpec(
        ...
        kmesh=kmesh,
        params=_default_params() if not params_json else QEParams(**json.loads(params_json)),
...
def run_qe_local(job_json: str) -> Dict[str, Any]:
    ...
    out_text = Path(outpath).read_text(errors="ignore") if Path(outpath).exists() else ""
    parsed = _parse_scf(out_text)
    return {"returncode": rc, "output_path": outpath, "parsed": parsed, "stdout_tail": out_text[-2000:]}
...
def run_qe_aiida(job_json: str) -> Dict[str, Any]:
    ...
    if not _aiida_available():
        return {"submitted": False, "reason": "AiiDA not available in this environment."}
    ...
    inputs = {
        "code": Code.get_from_string("qe-pw@localhost"),  # user must configure this label
        "structure": structure,
        "kpoints": kpoints,
        "parameters": Dict(dict={
            "CONTROL": job.params.control if job.params else _default_params().control,
            "SYSTEM": job.params.system if job.params else _default_params().system,
            "ELECTRONS": job.params.electrons if job.params else _default_params().electrons,
        }),
