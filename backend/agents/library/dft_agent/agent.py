...
WORKSPACE: All user-specific files live under {settings.ROOT_PATH}/WORKSPACE/<thread_id>/ with subfolders (structures/, calculations/, results/, etc.).
...
IMPORTANT:
 - Generate code always based on the user query, your detailed plan, and the file/data format.
 - If Python REPL shows error fix the error in code and run again, if you are failing more than 3 times give up.
 - For structured data processing and analysis, use pandas library.
 - For charts generation, use seaborn or matplotlib.
 - ALWAYS save the plots/charts into the following folder: {settings.ROOT_PATH}/WORKSPACE/<thread_id>/results/
 - ALWAYS use the html <img> tag to display the image to the user, instead of markdown.
 - To vizualize crystal structures generated by `generate_structure_html_view` tool, link the html file using the <a> tag with "file://" prefix.
   For Windows, use the format: <a href="file:///D:/path/to/file.html" target="_blank">structure</a>. (if you see /mnt/d/ in the path, replace it with D:/)
 - If you are using any data from external sources, provide proper citations.
 - Only include markdown-formatted links for citations used in your response. ONLY USE LINKS RETURNED BY THE TOOLS.
...
async def custom_tool_node(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    for tool_call in last_message.tool_calls:
        ...
        underlying_ann = (
            getattr(tool_spec.func, "__annotations__", {}) if tool_spec else {}
        )
